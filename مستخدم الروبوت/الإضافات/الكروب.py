...
async def tmuter(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("**☭ ⦙ رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @MedThon**")
...
async def iq(event):
    ...
    if type_of_group == "قناه":
        descript = "☭ ⦙   هذه قناة إختبار أُنشئت بإستعمال مد ثون"
    else:
        descript = "☭ ⦙   هذه المجموعه إختبار أُنشئت بإستعمال مد ثون"
