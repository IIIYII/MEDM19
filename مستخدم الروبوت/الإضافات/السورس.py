...
IQTHONPC = gvarstatus("ALIVE_PIC") or "https://telegra.ph/file/9e9e1cd623335a9c5772d.jpg"
...
async def _(event):
    ...
    OUTPUT = (f"☭︙  وحـدة المعلومات الخاصه بتنصيبك مع جميع الفارات  لتنصيب سورس مد ثون@iqthon :**\n\n{o}")
...
async def _(event):
    if BOTLOG:
        await event.client.send_message(BOTLOG_CHATID, "**☭︙   تم تحديث سورس مد ثون ↻**")
    sandy = await edit_or_reply(event , "☭︙  جـاري تـحديـثمد ثون🔄\n🔹 - قـد يستغـرق الأمـر 5 - 10 دقائـق انتـظـر\nلاتقـم بتحـديث أكثـر من 3 مـرات باليـوم" ,)
    try:
    ...
    if vname in vlist:
        ...
        if cmd == "اضف":
            if not vinfo and vname == "ALIVE_TEMPLATE":
                return await edit_delete(event, f"**☭︙  📑 يرجى متابع قناه السورس تجدها هنا :@iqthon")
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inlineiqthon(iqthon):
        ...
        if query.startswith("تنصيب") and iqthon.query.user_id == bot.uid:
            buttons = [[Button.url("1- قناة السورس", "https://t.me/MedThon"), Button.url("2- استخراج ايبيات", "https://my.telegram.org/"),],[Button.url("3- ستخراج تيرمكس", "https://replit.com/@telethon-Arab/generatestringsession#start.sh"), Button.url("4- بوت فاذر", "http://t.me/BotFather"),],[Button.url("5- رابط التنصيب", "https://dashboard.heroku.com/new?template=https://github.com/WALEUSERBOT/WALEUSERHELP"),],[Button.url("المطـور 👨🏼‍💻", "https://t.me/oipu1"),]]
...
async def iq(event):
    await edit_or_reply(event, """✐  ⦗ اوامـر سـورس مد ثون⦘
                                             ┉┉┉┉┉┉×┉┉┉┉┉
〖`.م1`〗⏎  اوامر الحساب 1
〖`.م2`〗⏎  اوامر الحساب 2
〖`.م3`〗⏎  اوامر الحساب 3
